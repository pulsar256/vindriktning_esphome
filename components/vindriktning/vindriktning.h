...
class VindriktningSensor : public PollingSensorComponent, public UARTDevice
{
  ...
  char packetHeader[3] = {0x16, 0x11, 0x0b};
  long lastTime = millis();
  int bufferPos = 0;
  short reading = 0;

public:
  VindriktningSensor(UARTComponent *parent) : PollingSensorComponent("pm25sensor", 5000), UARTDevice(parent) {}
...
private:
  void fillBuffer(char *c) {
    long currentTime = millis();
        ...
        if (delta > 800)
        {
          ESP_LOGV(TAG, "flushing buffer, current bufferPos %u, time delta %d", this->bufferPos, delta);
          bufferPos = 0;
        }
  ...
  void parseBuffer()
  {
    ...
    ESP_LOGD(TAG, "time: %d, reading: %d, buffer=[%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X]",
             millis(),
             reading,
             buffer[0],
             buffer[1],
